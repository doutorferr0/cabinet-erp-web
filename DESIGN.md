...
| 4 | `src/components/vitra/data-table.tsx` | **Grade fechada**: caixa em Régua, malha em Fio entre linhas **e colunas**; cabeçalho 36px em Meta com sublinha forte; marcador de 2px na linha selecionada — **feito** · coluna de numeração opcional — **feito** (prop `rowNumbers`: 40px, Meta à direita, sequencial global da consulta; desligada em listagem) · cabeçalho agrupado — **feito** (grupos do TanStack: `colSpan`, rótulo centralizado, fileira do grupo separada por Fio) |
...
| 6 | `src/components/vitra/form-grid.tsx` | Mesma malha da DataTable — **feito** (herdada dos primitivos `ui/table` na grade fechada do item 4) · faixa de seção para agrupamento de linhas — **feito** (prop `sectionKey`: banda de largura total, rótulo em Meta, régua forte acima e abaixo, fundo Bancada; adoção no `Ambiente` do orçamento aguarda a captura confirmar o agrupamento) |
| 7 | `src/components/vitra/documento.tsx` | `DocumentoHeader` com número em mono — **feito** (3 telas de documento migradas) · **totais migram de tira separada para as últimas fileiras da grade de itens**, valor sob a coluna de valor — **feito** (prop `totals` do FormGrid + helper `fileirasTotais`: rótulo em Meta na coluna anterior, valor sob a coluna de valor, Total com régua forte acima e único em Title; ajustes do pedido/ordem/orçamento viraram fileiras; campos Desconto/Acréscimo da ordem e abas Impostos/Frete do orçamento permanecem fora da grade — a tira `DocumentoTotais` fica como a exceção documentada para tela sem grade) |
| 8 | novo | Componente `Stamp` — **feito** (`stamp.tsx`, 4 tons por propriedade; mapeamento tom → situação continua `[a resolver]`) |
| 9 | `src/components/ui/` | Mensagem de validação de `0.8rem` para `0.75rem` — **feito** (já entrou na V11, commit `4d99287`: `FormMessage` e `FormDescription` em `text-xs`; asserção de regressão no teste do produto) |

Itens 1–9 implementados (tokens nos dois temas, raio de 4px, sidebar sem paleta própria, anel em Régua Forte — contrastes verificados por cálculo WCAG; moldura `page-frame` no shell com superfícies Documento corrigidas; DataTable ledger completa — grade fechada, cabeçalho Meta 36px, numéricas tabulares, seleção com marcador, numeração opcional via `rowNumbers` e cabeçalho agrupado; `FormBlock` com `<legend>` em Meta nos 9 fieldsets, rodapé do form com régua forte e todos os glifos emoji substituídos por ícones Lucide; FormGrid com a mesma malha da DataTable, faixa de seção via `sectionKey` — adoção no orçamento aguarda confirmação da captura — e totais como últimas fileiras da grade via prop `totals` + `fileirasTotais`, nas 3 telas de documento; `DocumentoHeader` com número em mono; `Stamp` existe; validação em `0.75rem` desde a V11). Pendências que NÃO são itens: adoção da faixa de seção no orçamento (captura) e mapeamento tom → situação do carimbo (`[a resolver]`).

O sidecar `.impeccable/design.json` foi **regenerado do código novo** (V20) depois dos itens 1–9: tokens dos dois temas, componentes com a gramática papel funcional (moldura, grade fechada, Meta, totais no pé, carimbo) e narrativa "Papel Funcional". Ele volta a ser um retrato fiel do sistema implementado — este arquivo continua sendo a fonte canônica da direção.
