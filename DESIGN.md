...
Itens 1–6, 8 e 9 implementados; resta apenas o resto do item 7 (tokens nos dois temas, raio de 4px, sidebar sem paleta própria, anel em Régua Forte — contrastes verificados por cálculo WCAG; moldura `page-frame` no shell com superfícies Documento corrigidas; DataTable ledger completa — grade fechada, cabeçalho Meta 36px, numéricas tabulares, seleção com marcador, numeração opcional via `rowNumbers` e cabeçalho agrupado; `FormBlock` com `<legend>` em Meta nos 9 fieldsets, rodapé do form com régua forte e todos os glifos emoji substituídos por ícones Lucide; FormGrid com a mesma malha da DataTable e faixa de seção via `sectionKey` — adoção no orçamento aguarda confirmação da captura; `DocumentoHeader` com número em mono nas 3 telas e tira de totais com régua forte — falta a migração dos totais para as fileiras da grade, que depende de decisão: totais na tira × nas fileiras da grade; `Stamp` existe; validação em `0.75rem` desde a V11). **Não afirmar em nenhum relatório que o sistema "papel funcional" está implementado até que os itens acima existam em código.**
