...
border-radius:6px;background:var(--papel);color:var(--tinta);cursor:pointer}
...
<button onclick="limpaTudo()" title="remove todas as tabelas do canvas">✕ limpar</button>
<div class="legenda">
...
function carrega(){ try{ const s=localStorage.getItem(chaveStore());
  if(s){ const j=JSON.parse(s); aberto=j.aberto||{}; view=j.view||view; return true; } }catch(e){}
  return false; }
...
function limpaTudo(){ aberto={}; render(); }
function vizinhos(t){
