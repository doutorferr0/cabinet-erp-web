...
describe('VitraDataTable — agrupamento na grade', () => {
  ...
  it('agrupa por Situação com contagem e subtotal em Money', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /nenhum agrupamento aplicado/ }))
    await user.click(await screen.findByRole('menuitem', { name: 'Situação' }))
  ...
  it('agrupar por campo SEM tom não pinta nada — nome próprio não é estado', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /nenhum agrupamento aplicado/ }))
    await user.click(await screen.findByRole('menuitem', { name: 'Fornecedor' }))
  ...
  it('campo sem `valorDaLinha` não entra no chip — a tabela não sabe lê-lo', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /nenhum agrupamento aplicado/ }))
    expect(await screen.findByRole('menuitem', { name: 'Fornecedor' })).toBeInTheDocument()
    expect(screen.queryByRole('menuitem', { name: 'Etapa' })).not.toBeInTheDocument()
  })
})
