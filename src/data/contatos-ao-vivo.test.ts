...
describe.skipIf(!process.env.CABINET_AO_VIVO)('contatos contra o backend real', () => {
  ...
  it('a leitura devolve só os ATIVOS, e o total do servidor denuncia o resto', async () => {
    ...
    // igualzinho, então o que distingue é o conteúdo. Aqui a diferença entre
    // `linhas.length` e `total` É o contato desativado que ficou de fora.
    const { linhas, total, cortou } = await listarContatos(partnerId)

    expect(total).toBeGreaterThanOrEqual(linhas.length)
  ...
  it('inclui, relê do servidor e remove — o ciclo inteiro da grade', async () => {
    ...
    await sincronizarContatos(partnerId, antes.linhas, [
      ...antes.linhas,
      {
        id: null,
        nome: MARCA,
    ...
    const criado = comONovo.linhas.find((l) => l.nome === MARCA)
    ...
    await sincronizarContatos(
      partnerId,
      comONovo.linhas,
      comONovo.linhas.map((l) => (l.id === criado?.id ? { ...l, fone: '1140004000' } : l)),
    )
    const alterado = (await listarContatos(partnerId)).linhas.find((l) => l.id === criado?.id)
    ...
    const semEle = comONovo.linhas.filter((l) => l.id !== criado?.id)
    await sincronizarContatos(partnerId, comONovo.linhas, semEle)

    const depois = await listarContatos(partnerId)
    expect(depois.linhas.map((l) => l.id)).not.toContain(criado?.id)
    // Removido da grade, PRESENTE no servidor: desativação lógica, não exclusão.
    expect(depois.total).toBe(comONovo.total)
  })
