...
describe('passthrough por rota', () => {
  ...
  it('a ÚNICA sem-contrato é o reagendamento do Planner', () => {
    // ESTE CASO É O CONTADOR DA DÍVIDA, e ele já valeu 0, 1, 26 e agora 1 de
    // novo. O comentário anterior previa o encolhimento — *"quando a PR do api
    // sincronizar e ligar os handlers, este caso encolhe de novo"* — e o dia
    // chegou: a `api#229` rodou o `sync:contract` (196 → 199 operações) e as
    // vinte e cinco deixaram de ser 404 do roteador.
    //
    ...
    ).toEqual(['patch /api/projects/{projectId}/plan/items/{itemId}'])
    // Cabeçalho com o próximo passo + uma linha por rota = 1 + 1.
