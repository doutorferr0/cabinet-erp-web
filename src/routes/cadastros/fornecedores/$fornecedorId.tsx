 ...
 */
function fornecedorDoParceiro(dto: PartnerDto): Fornecedor {
  return {
    ...fornecedorVazio(0),
...
function FornecedorEditPage() {
  ...
  const [linha] = useState<PartnerDto | null>(
    () => queryClient.getQueryData<PartnerDto>(['parceiro', fornecedorId]) ?? null,
  )

  const gravar = useMutation({
    mutationFn: (values: Fornecedor) => {
  ...
  if (!registro) {
    return (
      <p className="max-w-prose text-muted-foreground">
        Abra o fornecedor pela listagem. O contrato tem <code>PUT /api/partners/{'{id}'}</code> mas
        não tem leitura por id, então é a linha selecionada que traz o registro — link direto e
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <FornecedorForm
        fornecedor={registro}
        readOnly={readOnly}
        {...(isNovo ? {} : { onGravar: (v: Fornecedor) => gravar.mutate(v) })}
      />
    </div>
  )
}
    ...
    <div className="flex flex-col gap-1">
      <p className="max-w-prose text-[0.75rem] text-muted-foreground">
        {isNovo ? (
          ...
          <>
            <strong>Gravar</strong> envia ao servidor apenas Razão Social, Nome Fantasia, CNPJ/CPF,
